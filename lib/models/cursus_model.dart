...
/*
class ListCursus{
  List<Cursus> listCurs;
  ListCursus({this.listCurs});
  factory ListCursus.fromJson(List<dynamic> json){
    List <Cursus> boucle= new List<Cursus>();
    boucle=json.map((f)=>Cursus.fromJson(f)).toList();
    return ListCursus(
      listCurs: boucle,
    );
  }
...
class Cursus {
  ...
  final String nom;
  final String image;
  //final List<Map> level;
  Cursus({this.id, this.nom, this.image,});
